...
// 🌟 樣式工廠：必須要有這一行 export const getStyles！
export const getStyles = (isDarkMode = false) => {
  const colors = {
    bgMain: isDarkMode ? '#0f172a' : '#f0f2f5',
    bgCard: isDarkMode ? '#1e293b' : '#ffffff',
    bgPanel: isDarkMode ? '#1e293b' : '#ffffff',
    bgHeader: isDarkMode ? '#020617' : '#20353f',
    bgSoft: isDarkMode ? '#334155' : 'rgba(211, 224, 227, 0.45)',
    bgInput: isDarkMode ? '#0f172a' : '#ffffff',
    bgChip: isDarkMode ? '#334155' : '#f1f5f9',
    bgModalTop: isDarkMode ? '#0f172a' : '#fafafa',
    
    textMain: isDarkMode ? '#f8fafc' : '#1a2a3a',
    textBody: isDarkMode ? '#cbd5e1' : '#1f2937',
    textMuted: isDarkMode ? '#94a3b8' : '#64748b',
    
    border: isDarkMode ? '#334155' : '#e2e8f0',
    borderDark: isDarkMode ? '#475569' : '#cbd5e1',
    
    primary: isDarkMode ? '#38bdf8' : '#376171',
    accent: isDarkMode ? '#2dd4bf' : '#0f766e',
    
    activeBg: isDarkMode ? '#38bdf8' : '#111827',
    activeText: isDarkMode ? '#0f172a' : '#ffffff',

    gradientHint: isDarkMode 
      ? 'linear-gradient(to bottom, rgba(30,41,59,0) 0%, rgba(30,41,59,0.95) 100%)' 
      : 'linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,0.95) 100%)'
  };
  ...
  return StyleSheet.create({
    ...
    header: { backgroundColor: colors.bgHeader, paddingVertical: 14, paddingHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', zIndex: 100, flexWrap: 'nowrap' },
    ...
    headerLangContainer: { flexShrink: 0, flexDirection: 'row', alignItems: 'center' },
    ...
    emptyText: { textAlign: 'center', marginTop: 50, fontSize: 16, color: colors.textMuted, cursor: 'default' },
    
    searchBarSection: { paddingHorizontal: 20, paddingVertical: 20, backgroundColor: colors.bgCard, alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.border, zIndex: 9999 },
    mainControlContainer: { width: '100%', maxWidth: 1400, flexDirection: 'column', gap: 16 },
    
    topSearchSection: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bgSoft, borderRadius: 24, paddingVertical: 12, paddingHorizontal: 25, zIndex: 10000, flexWrap: 'wrap', gap: 10 },
    searchBarMainTitle: { fontSize: 17, fontWeight: 'bold', color: colors.textMain, letterSpacing: 1, cursor: 'default' },
    verticalDivider: { width: 1, height: 24, backgroundColor: colors.borderDark, marginHorizontal: 16 },
    ...
    dropdownBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.bgCard, borderRadius: 20, paddingHorizontal: 14, height: 38, borderWidth: 1, borderColor: colors.border, minWidth: 160, cursor: 'pointer' },
    dropdownBtnText: { fontSize: 13, color: colors.textMain, fontWeight: '500', flex: 1 },
    dropdownArrow: { fontSize: 10, color: colors.textMuted, marginLeft: 8 },
    dropdownList: { position: 'absolute', top: 42, left: 0, width: 220, backgroundColor: colors.bgCard, borderRadius: 12, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 8, elevation: 5, paddingVertical: 6, borderWidth: 1, borderColor: colors.border, zIndex: 10002 },
    ...
    dropdownScrollHint: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 35, backgroundImage: colors.gradientHint, justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 6, borderBottomLeftRadius: 12, borderBottomRightRadius: 12 },
    dropdownScrollHintText: { fontSize: 10, color: colors.textMuted, fontWeight: 'bold', letterSpacing: 1 },

    searchInputWrapper: { flex: 1, minWidth: 260, maxWidth: 350, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bgInput, borderRadius: 20, paddingLeft: 14, paddingRight: 6, height: 38, borderWidth: 1, borderColor: colors.border },
    officialSearchInput: { flex: 1, height: '100%', fontSize: 16, color: colors.textMain, outlineWidth: 0 },
    ...
    bottomFilterSection: { backgroundColor: colors.bgSoft, borderRadius: 24, paddingVertical: 16, paddingHorizontal: 25, zIndex: 900 },
    
    dragHandleContainer: { width: '100%', alignItems: 'center', paddingBottom: 10, paddingTop: 0, marginTop: -6 },
    dragHandlePill: { width: 40, height: 5, backgroundColor: colors.borderDark, borderRadius: 3 },
    ...
    panelMainTitle: { fontSize: 17, fontWeight: 'bold', color: colors.textMain, letterSpacing: 1 }, 
    panelToggleIndicatorText: { fontSize: 13, color: colors.textMuted, fontWeight: '600' },
    ...
    filterLeftColumnBorder: { borderRightWidth: 1, borderRightColor: colors.borderDark, paddingRight: 20 },
    ...
    panelLabel: { fontSize: 13, fontWeight: 'bold', color: colors.textMuted, width: 75, whiteSpace: 'nowrap', flexShrink: 0, cursor: 'default' },
    rangeTrackLabel: { width: 50, fontSize: 13, fontWeight: 'bold', color: colors.textMuted, cursor: 'default', flexShrink: 0 },
    rangeTrackBox: { flexDirection: 'row', backgroundColor: colors.bgChip, borderRadius: 15, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' },
    ...
    rangeNumberText: { fontSize: 13, color: colors.textMuted, fontWeight: 'bold' },
    rangeNumberTextActive: { color: '#ffffff' },
    trackResetBtn: { marginLeft: 10, width: 24, height: 24, borderRadius: 12, backgroundColor: colors.bgChip, justifyContent: 'center', alignItems: 'center', cursor: 'pointer' },
    trackResetBtnText: { fontSize: 14, color: colors.textMuted, fontWeight: 'bold', lineHeight: 24 },
    ...
    filterChip: { paddingHorizontal: 11, paddingVertical: 5, borderRadius: 12, backgroundColor: colors.bgChip, borderWidth: 1, borderColor: colors.border, cursor: 'pointer', whiteSpace: 'nowrap' },
    chipText: { fontSize: 12, color: colors.textMuted, fontWeight: '500' },
    
    reprintTickbox: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, backgroundColor: colors.bgChip, borderWidth: 1, borderColor: colors.borderDark, flexDirection: 'row', alignItems: 'center', cursor: 'pointer', whiteSpace: 'nowrap' },
    reprintTickboxActive: { backgroundColor: '#10b981', borderColor: '#059669' },
    reprintTickboxText: { fontSize: 12, color: colors.textMuted, fontWeight: 'bold' },
    reprintTickboxTextActive: { color: '#ffffff' },

    filterChipWithInput: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bgChip, borderRadius: 12, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' },
    filterChipInner: { paddingHorizontal: 11, paddingVertical: 5, cursor: 'pointer' },
    keywordValInput: { width: 24, height: '100%', fontSize: 16, textAlign: 'center', color: colors.textMain, borderLeftWidth: 1, borderLeftColor: colors.borderDark, outlineWidth: 0 },
    keywordValInputActive: { color: '#ffffff', borderLeftColor: colors.borderDark },

    traitShortInput: { width: 180, height: 32, backgroundColor: colors.bgInput, borderRadius: 16, paddingHorizontal: 14, borderWidth: 1, borderColor: colors.borderDark, fontSize: 16, outlineWidth: 0, color: colors.textMain, marginRight: 8 },
    ...
    exactMatchBtn: { marginLeft: 8, paddingHorizontal: 8, height: 32, borderRadius: 16, borderWidth: 1, borderColor: colors.borderDark, justifyContent: 'center', alignItems: 'center', backgroundColor: colors.bgChip, cursor: 'pointer', whiteSpace: 'nowrap' },
    exactMatchBtnActive: { backgroundColor: colors.activeBg, borderColor: '#000000' },
    exactMatchBtnText: { fontSize: 11, color: colors.textMuted, fontWeight: 'bold', letterSpacing: 0.5 },
    exactMatchBtnTextActive: { color: colors.activeText },
    ...
    gridCard: { backgroundColor: colors.bgCard, borderRadius: 8, margin: 5, overflow: 'hidden', shadowColor: '#000', shadowOpacity: isDarkMode ? 0.4 : 0.1, shadowRadius: 3, elevation: 2, cursor: 'pointer', transition: 'transform 0.2s ease, box-shadow 0.2s ease', transform: [{ translateY: 0 }] },
    gridCardHovered: { transform: [{ translateY: -4 }], shadowOpacity: isDarkMode ? 0.6 : 0.25, shadowRadius: 8, elevation: 6 },
    imageWrapper: { width: '100%', backgroundColor: colors.bgChip, position: 'relative' },
    gridImage: { width: '100%', height: '100%' },
    gridNoImage: { flex: 1, justifyContent: 'center', alignItems: 'center' },
    gridNoImageText: { color: colors.textMuted, fontSize: 10, fontWeight: 'bold', cursor: 'default' },
    gridCardInfo: { padding: 6 },
    gridIdRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 },
    gridCardId: { flex: 1, fontSize: 11, fontWeight: 'bold', color: colors.textMuted, cursor: 'default' },
    ...
    gridCardName: { fontSize: 13, fontWeight: 'bold', color: colors.textMain, cursor: 'default' },
    
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(10px)', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 20, position: 'relative' },
    modalContentBox: { backgroundColor: colors.bgPanel, width: '100%', maxWidth: 920, maxHeight: '92%', borderRadius: 12, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.35, shadowRadius: 15, elevation: 10, mx: 20, display: 'flex', flexDirection: 'column', flexShrink: 1 },
    ...
    modalTopBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border, backgroundColor: colors.bgModalTop, flexShrink: 0 },
    
    modalHeaderIdBox: { flexDirection: 'row', alignItems: 'center' },
    modalHeaderId: { fontSize: 14, color: colors.textMuted, marginRight: 10, fontFamily: 'monospace', cursor: 'default' },
    modalHeaderRarity: { fontSize: 12, fontWeight: 'bold', backgroundColor: colors.bgChip, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, color: colors.textBody, cursor: 'default' },
    ...
    closeButton: { width: 34, height: 34, backgroundColor: colors.bgChip, borderRadius: 17, justifyContent: 'center', alignItems: 'center', cursor: 'pointer' },
    closeButtonText: { fontSize: 16, fontWeight: 'bold', color: colors.textBody },
    ...
    noImagePlaceholder: { backgroundColor: colors.bgChip, justifyContent: 'center', alignItems: 'center' },
    noImageText: { color: colors.textMuted, fontSize: 18, fontWeight: 'bold', cursor: 'default' },
    
    officialNameMain: { fontSize: 28, fontWeight: 'bold', color: colors.textMain, marginBottom: 15, letterSpacing: 0.5, cursor: 'default' },
    
    officialSpecsRow: { flexDirection: 'row', paddingBottom: 15, borderBottomWidth: 1, borderBottomColor: colors.borderDark, marginBottom: 15 },
    specItem: { fontSize: 15, color: colors.textBody, marginRight: 25, fontWeight: '500', cursor: 'default' },
    specLabel: { color: colors.textMuted, fontWeight: 'normal', marginRight: 4 },
    officialSectionDivider: { paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: colors.border, marginBottom: 12 },
    sectionTitle: { fontSize: 13, fontWeight: 'bold', color: colors.accent, marginBottom: 6, letterSpacing: 0.5, cursor: 'default' }, 
    sectionBodyText: { fontSize: 15, color: colors.textBody, lineHeight: 24, cursor: 'default' }, 
    statsRow: { flexDirection: 'row' },
    
    interactiveBoldToken: { fontWeight: 'bold', color: colors.primary, cursor: 'pointer' }, 
...
// 🌟 防呆機制：預設輸出「亮色模式」給還沒更新的舊組件，保證系統不崩潰！
